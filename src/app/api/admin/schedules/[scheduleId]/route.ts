...
export async function PUT(
  ...
  try {
    ...
    const { courseId, dayOfWeek, startTime, endTime, room, instructor, maxCapacity } = body

    // Validate required fields
    if (!courseId || !dayOfWeek || !startTime || !endTime || !room) {
      return NextResponse.json(
        { error: 'Course, day, time, and room are required' },
        { status: 400 }
      )
    }
    ...
    // Check for scheduling conflicts (same room, day, overlapping time, excluding current schedule)
